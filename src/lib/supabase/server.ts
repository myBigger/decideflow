...
// 声明 Supabase 数据库类型（与 client.ts 保持一致）
type Database = {
  public: {
    Tables: {
      teams: {
      ...
      decisions: {
        Row: {
          ...
          voting_start: string | null; voting_end: string | null
          pass_threshold: number; total_score: number | null
          final_result: string | null; round: number
          ai_insight: unknown; created_at: string; updated_at: string
        }
        Insert: Record<string, unknown>; Update: Record<string, unknown>
      }
      options: {
      ...
      executions: {
        Row: {
          id: string; decision_id: string; title: string
